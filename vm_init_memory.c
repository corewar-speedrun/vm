...
/*
** vm_init_car создает каретку в начале игры или при выполнении команды fork
** lfork. в последнем случае копирует содержимое регистров и необходимых
** значений родительской каретки в дочернюю.
*/
...
/*
** vm_map_write - при выполнениии команды, во время игры, которая може
** писать на карту, эта ф-ци отвечает непосредственно за запись данных.
** g_vm->map0 - сюда пишутся изменения игрового поля.
** g_vm->map1 - сюда пишется акой бот ти изменения на поле сделал 
** g_vm->map3 - сюда отмечается, что эти точки поля надо будет подсвечивать 
** ближайшие 100 циклов, как измененные.
** g_vm->map1 и g_vm->map3 используются для визуализации.
*/
