...
** vm_read_comand
**
** считываем с поля и парсим команду каретке.
** если команда валидна, из следующей ф-ции (vm_cycles_count) подтягиваем
** значение цикла ожидания выполнения команды. и если это команда лайв, помечаем
** каретке флаг лайв.
** 
** в ином случае записываем себе, что каретку нужно передвинуть на следующую
** ячейку памяти и попытаться снова на следующем цикле.
*/
...
/*
** vm_make_move и vm_make_move2 отвечают за запуск выполнения соответствующей
** имени/номеру комнды
*/
...
/*
** vm_finish_game - если боты доиграли до конца, задача ф-ци найти какой бот
** последним сказал лайв. кто был последним, тот и победитель.
** еслилайв сказали 2 бота одновременно, выигрывает первый в списке бот. играть
** и ходить начинает последний бот. первый бот ходит последним. потому
** выигрывает первый в списке бот, который, по факту, ходил последним.
*/
